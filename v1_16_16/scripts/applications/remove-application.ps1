...
[CmdletBinding()]param([Parameter(Mandatory=$true)][string]$Name,[switch]$Apply)
...
$a=@((Join-Path $Root 'scripts\applications\application-helper.py'),'remove','--name',$Name); if($Apply){$a+='--apply'}; if($Python.Name -like 'py*'){$a=@('-3')+$a}; & $Python.Source @a; if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}
